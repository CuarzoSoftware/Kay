...
void MLayerSurface::Imp::createRole() noexcept
{
    ...
    const Int32 finalW = requestAvailableWidth && pendingAnchor.checkAll(AKEdgeLeft | AKEdgeRight) ? 0 : (w != 0 ? w : 8);
    const Int32 finalH = requestAvailableHeight && pendingAnchor.checkAll(AKEdgeTop | AKEdgeBottom) ? 0 : (h != 0 ? h : 8);
    ...
    if (zwlr_layer_surface_v1_get_version(layerSurface) >= 5)
    {
        zwlr_layer_surface_v1_set_exclusive_edge(
            layerSurface,
            currentAnchor.check(currentExclusiveEdge) ? currentExclusiveEdge : AKEdgeNone);
    }
