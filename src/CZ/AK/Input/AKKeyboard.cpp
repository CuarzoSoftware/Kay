...
void AKKeyboard::loadComposeTable(const char *locale) noexcept
{
    ...
    AKLog(CZInfo, "[AKKeyboard] Using locale %s.", locale);
    return;
fail:
    // Try with uppercase
    if (locale)
    {
        ...
        {
            AKLog(CZWarning, "[AKKeyboard] Failed to create compose table from locale %s. Trying with %s.", locale, localeUpper.c_str());
    ...
    {
        AKLog(CZWarning, "[AKKeyboard] Failed to create compose table from locale %s. Falling back to locale C.", locale);
        loadComposeTable("C");
    }
}
