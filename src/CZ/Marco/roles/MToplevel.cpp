...
{
    return states().has(AKFullscreen);
}
...
CZBitset<AKWindowState> MToplevel::states() const noexcept
{
...
bool MToplevel::eventFilter(const CZEvent &event, AKObject &object)
{
    if (rootNode() == &object)
    {
...
bool MToplevel::event(const CZEvent &e)
{
    if (e.type() == CZEvent::WindowClose)
    {
...
void MToplevel::onUpdate() noexcept
{
    ...
    if (states().has(AKFullscreen) || decorationMode() == ServerSide || !builtinDecorationsEnabled())
    {
...
void MToplevel::render() noexcept
{
    ...
    if (MSurface::imp()->backgroundBlur && app()->wayland().svgPathManager)
    {
        if (vibrancyState() == AKVibrancyState::Enabled && opacity() < 1.f)
        {
            ...
            int r = !builtinDecorationsEnabled() || states().has(AKFullscreen) ? 0 : MTheme::CSDBorderRadius;
