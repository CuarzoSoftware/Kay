...
MSurface::MSurface(Role role) noexcept : CZ::AKSolidColor(SK_ColorWHITE)
{
    ...
    target()->on.markedDirty.subscribe(this, [this](CZ::AKTarget&){
        update();
    });
...
void MSurface::vibrancyEvent(const AKVibrancyEvent &event)
{
    ...
    {
        if (it.node() != this)
            queue.addEvent((const CZEvent&)event, (AKObject&)(*it.node()));
...
bool MSurface::event(const CZEvent &event)
{
    if (event.type() == CZEvent::Vibrancy)
    {
