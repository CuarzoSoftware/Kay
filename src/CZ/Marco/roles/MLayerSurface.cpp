...
void MLayerSurface::render() noexcept
{
    ...
    if (sizeChanged || anchorChanged)
    {
        ...
        const Int32 finalW = imp()->requestAvailableWidth && anchor().checkAll(CZEdgeLeft | CZEdgeRight) ? 0 : newSize.width();
        const Int32 finalH = imp()->requestAvailableHeight && anchor().checkAll(CZEdgeTop | CZEdgeBottom) ? 0 : newSize.height();
