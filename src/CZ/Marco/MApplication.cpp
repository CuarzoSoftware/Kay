...
#include <CZ/AK/CZBooleanEventSource.h>
...
MPointer &CZ::pointer() noexcept { return app()->pointer(); }
MKeyboard &CZ::keyboard() noexcept { return app()->keyboard(); }
...
void MApplication::wl_pointer_enter(void */*data*/, wl_pointer */*pointer*/, UInt32 serial, wl_surface *surface, wl_fixed_t x, wl_fixed_t y)
{
    ...
    CZCore::Get()->sendEvent(p.m_eventHistory.enter, surf->scene());
}
...
void MApplication::wl_pointer_leave(void */*data*/, wl_pointer */*pointer*/, UInt32 serial, wl_surface *surface)
{
    ...
    p.m_eventHistory.button.setState(CZPointerButtonEvent::Released);
    p.m_eventHistory.button.assignCurrentTime();

    while (!p.m_pressedButtons.empty())
    {
        p.m_eventHistory.button.setButton((CZPointerButtonEvent::Button)*p.m_pressedButtons.begin());
...
void MApplication::wl_pointer_motion(void */*data*/, wl_pointer */*pointer*/, UInt32 time, wl_fixed_t x, wl_fixed_t y)
{
    ...
    CZCore::Get()->sendEvent(p.m_eventHistory.move, p.focus()->scene());
...
void MApplication::wl_pointer_axis(void */*data*/, wl_pointer *pointer, UInt32 time, UInt32 axis, wl_fixed_t value)
{
    ...
    else if (p.focus())
    {
        ...
        p.m_eventHistory.scroll.setSource(CZPointerScrollEvent::Continuous);
        ...
        CZCore::Get()->sendEvent(p.m_eventHistory.scroll, p.focus()->scene());
    }
}
...
void MApplication::wl_pointer_frame(void */*data*/, wl_pointer */*pointer*/)
{
    ...
    p.m_framedScrollEvent = CZPointerScrollEvent({0.f, 0.f}, {0, 0}, false, false, CZPointerScrollEvent::Identical, CZPointerScrollEvent::Identical, CZPointerScrollEvent::Continuous);
...
void MApplication::wl_pointer_axis_source(void */*data*/, wl_pointer *pointer, UInt32 axis_source)
{
    ...
    if (wl_pointer_get_version(pointer) < 8 && axis_source == WL_POINTER_AXIS_SOURCE_WHEEL)
        p.m_framedScrollEvent.setSource(CZPointerScrollEvent::WheelLegacy);
    else
        p.m_framedScrollEvent.setSource((CZPointerScrollEvent::Source)axis_source);
}
...
void MApplication::wl_pointer_axis_relative_direction(void */*data*/, wl_pointer */*pointer*/, UInt32 axis, UInt32 direction)
{
    ...
    if (axis == WL_POINTER_AXIS_HORIZONTAL_SCROLL)
        p.m_framedScrollEvent.setRelativeDirectionX((CZPointerScrollEvent::RelativeDirection)direction);
    else if (axis == WL_POINTER_AXIS_VERTICAL_SCROLL)
        p.m_framedScrollEvent.setRelativeDirectionY((CZPointerScrollEvent::RelativeDirection)direction);
}
...
void MApplication::wl_keyboard_key(void */*data*/, wl_keyboard */*keyboard*/, UInt32 serial, UInt32 time, UInt32 key, UInt32 state)
{
    ...
    event.setSerial(serial);
    event.setUs(CZ::AKTime::us());
    event.setMs(time);
    event.setKeyCode(key);
    event.setState((CZ::CZKeyboardKeyEvent::State)state);
...
void MApplication::initWayland() noexcept
{
    ...
    m_marcoSource = CZBooleanEventSource::Make(true, [this](auto) {
        updateSurfaces();
    });
