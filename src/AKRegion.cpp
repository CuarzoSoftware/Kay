...
void AKRegion::multiply(Float32 factor) noexcept
{
    if (factor == 1.f)
        return;
...
void AKRegion::multiply(Float32 xFactor, Float32 yFactor) noexcept
{
...
void AKRegion::transform(const AKSize &size, AKTransform transform) noexcept
{
    ...
    case AKTransform::Flipped270:
        pixman_region32_init(&tmp);
        boxes = (AKBox*)pixman_region32_rectangles(&m_region, &n);
        for (i = 0; i < n; i++)
        {
...
AKPointF AKRegion::closestPointFrom(const AKPointF &point, Float32 margin) const noexcept
{
    if (empty())
        return point;

    Float32 smallestDistance { std::numeric_limits<Float32>::max() };
    AKPointF closestPoint;
    Float32 distance;
    AKPointF tmpPoint;

    Int32 n;
    const AKBox *box = boxes(&n);
...
void AKRegion::multiply(AKRegion *dst, AKRegion *src, Float32 factor) noexcept
{
    if (dst == src)
    {
        src->multiply(factor);
