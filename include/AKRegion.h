 ...
 *
 * The AKRegion class provides an efficient mechanism for creating sets of rectangles that do not overlap in their geometries.
 ...
 * Internally, AKRegion employs the algorithm and functions from the [Pixman](http://www.pixman.org/) library.
 */
class AK::AKRegion
{
public:
    /**
     * @brief Constructor for creating an empty AKRegion.
     */
    AKRegion() noexcept
    {
     ...
     */
    AKRegion(const AKRect &rect) noexcept
    {
        pixman_region32_init_rect(&m_region, rect.x(), rect.y(), rect.w(), rect.h());
    }

    /**
     * @brief Destructor for freeing resources associated with AKRegion.
     */
    ~AKRegion() noexcept
    {
     ...
     *
     * @param other The AKRegion to copy from.
     */
    AKRegion(const AKRegion &other) noexcept
    {
        pixman_region32_init(&m_region);
     ...
     *
     * Constructs an AKRegion by moving the contents from another AKRegion, leaving the source AKRegion empty.\n
     * The source AKRegion remains valid and can be used afterwards.
     *
     * @param other The AKRegion to move from.
     */
    AKRegion(AKRegion &&other) noexcept
    {
     ...
     *
     * @param other The AKRegion to assign from.
     * @return A reference to the modified AKRegion.
     */
    AKRegion &operator=(const AKRegion &other) noexcept
    {
        if (&other != this)
            pixman_region32_copy(&m_region, &other.m_region);
        return *this;
     ...
     *
     * Assigns the contents of another AKRegion to this AKRegion by moving, leaving the source AKRegion empty.\n
     * The source AKRegion remains valid and can be used afterwards.
     *
     * @param other The AKRegion to move from.
     * @return A reference to this AKRegion after the move.
     */
    AKRegion &operator=(AKRegion &&other) noexcept
    {
        if (&other != this)
        {
            pixman_region32_fini(&m_region);
    ...
    /**
     * @brief Clears the AKRegion, deleting all rectangles.
     */
    void clear() noexcept
    {
    ...
    /**
     * @brief Adds a rectangle to the AKRegion (union operation).
     *
     * @param rect The rectangle to add.
     */
    void addRect(const AKRect &rect) noexcept
    {
     ...
     */
    void addRect(const AKPoint &pos, const AKSize &size) noexcept
    {
     ...
     * @param size The rectangle's size.
     */
    void addRect(Int32 x, Int32 y, const AKSize &size) noexcept
    {
     ...
     */
    void addRect(const AKPoint &pos, Int32 w, Int32 h) noexcept
    {
    ...
    /**
     * @brief Adds the content of another AKRegion to this AKRegion (union operation).
     *
     * @param region The AKRegion to add.
     */
    void addRegion(const AKRegion &region) noexcept
    {
        if (&region != this)
    ...
    /**
     * @brief Subtracts a rectangle from the AKRegion.
     *
     * @param rect The rectangle to subtract.
     */
    void subtractRect(const AKRect &rect) noexcept
    {
     ...
     * @param size The rectangle's size.
     */
    void subtractRect(const AKPoint &pos, const AKSize &size) noexcept
    {
        pixman_region32_t tmp;
     ...
     * @param h The height of the rectangle.
     */
    void subtractRect(const AKPoint &pos, Int32 w, Int32 h) noexcept
    {
        pixman_region32_t tmp;
     ...
     * @param size The rectangle's size.
     */
    void subtractRect(Int32 x, Int32 y, const AKSize &size) noexcept
    {
        pixman_region32_t tmp;
    ...
    /**
     * @brief Subtracts another AKRegion from this AKRegion.
     *
     * @param region The AKRegion to subtract.
     */
    void subtractRegion(const AKRegion &region) noexcept
    {
        pixman_region32_subtract(&m_region, &m_region, &region.m_region);
    }

    /**
     * @brief Intersects this AKRegion with another AKRegion.
     *
     * @param region The AKRegion to intersect with.
     */
    void intersectRegion(const AKRegion &region) noexcept
    {
        if (&region != this)
    ...
    /**
     * @brief Multiplies the components of each rectangle in the AKRegion by the given factor.
     *
    ...
    /**
    * @brief Check if the AKRegion contains a specific point.
     *
     * @param point The point to check.
     * @return true if the region contains the point, false otherwise.
     */
    bool containsPoint(const AKPoint &point) const noexcept
    {
        return pixman_region32_contains_point(&m_region, point.x(), point.y(), NULL);
    }

    /**
     * @brief Translate each rectangle in the AKRegion by the specified offset.
     *
     * @param offset The offset to apply.
     */
    void offset(const AKPoint &offset) noexcept
    {
     ...
     */
    void inverse(const AKRect &rect) noexcept
    {
        pixman_box32_t r {
            rect.x(),
            rect.y(),
    ...
    /**
     * @brief Check if the AKRegion is empty (contains no rectangles).
     *
    ...
    /**
     * @brief Clips the AKRegion to the area defined by the specified rectangle.
     *
     * @param rect The rectangle used for clipping.
     */
    void clip(const AKRect &rect) noexcept
    {
     ...
     * @brief Clips the AKRegion to the area defined by the specified rectangle.
     */
    void clip(const AKPoint &pos, const AKSize &size) noexcept
    {
    ...
    /**
     * @brief Get the extents of the AKRegion.
     *
     * This method returns a pointer to an LBox that represents the bounding box of the AKRegion.
     * The LBox contains the minimum and maximum coordinates that enclose the AKRegion.
     *
     * @return A pointer to the extents of the AKRegion as an LBox.
     */
    const AKBox &extents() const noexcept
    {
        return *(AKBox*)pixman_region32_extents(&m_region);
    }

    /**
     * @brief Retrieves the list of rectangles that form the AKRegion.
     *
     ...
     */
    const AKBox *boxes(Int32 *n) const noexcept
    {
        return (AKBox*)pixman_region32_rectangles(&m_region, n);
    }
     ...
     */
    void transform(const AKSize &size, AKTransform transform) noexcept;
     ...
     */
    AKPointF closestPointFrom(const AKPointF &point, Float32 padding = 0.f) const noexcept;
     ...
     */
    static const AKRegion &EmptyRegion() noexcept
    {
        static AKRegion emptyRegion;
        return emptyRegion;
    }

    static void multiply(AKRegion *dst, AKRegion *src, Float32 factor) noexcept;
